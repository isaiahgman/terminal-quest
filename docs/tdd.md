...
- `Enemy = { kind, pos, hp, maxHp, atk, def, speed, glyph, color }`  ·  `Boss = Enemy + { id, name, defeated }`  (*per-enemy AI bookkeeping is **not** a field on `Enemy`; it lives in a separate `EnemyAi` struct — see below. `glyph`/`color` are plain strings the renderer maps — see §12)
