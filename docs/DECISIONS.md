...
- **Enemy movement is the same accumulator, and needs the same clamp** — each enemy banks `speed * dt` and steps per whole tile (sub-tile speeds on a discrete grid). A blocked enemy can't spend its budget, so it must be clamped to one ready step (`<= 1`) or it hoards and teleports when freed — the per-enemy twin of the loop's spiral-of-death clamp. **Charge is a speed *multiplier* (`CHARGE_SPEED_MULTIPLIER`), not a per-tick lunge:** a floor flattens every kind to tick-rate speed and erases the speed stat in close range; a gain keeps the per-kind spread and stays tick-rate independent. (Both corrected in TQ-005 review — see TDD §12.)
- **Input — held-direction state at tick cadence, not "repeat on press".** We *thought* terminals have no key-up, forcing "repeat on press" (lean on OS auto-repeat) — but that has a built-in stall (the OS initial-repeat delay) and ties movement speed to the user's keyboard settings. TQ-016 replaced it: a keydown marks a direction "held," and the loop re-emits a move for each held direction every tick, so cadence is the game's, not the OS's. We then *found* the "no key-up" premise is outdated — modern terminals report real key-release via the **kitty keyboard protocol** — so the model is two-tier: timeout-inferred release as the legacy fallback (TQ-016), real key-release as the no-coast primary (TQ-019). Lesson: **verify a platform "constraint" before architecting around it** — this one had expired. (See TDD §12.)
...
- **Scope planning artifacts SMALLER.** Ours each map to ~3–5 real PRs. Keep PRs tiny; one artifact → several small PRs; never combine artifacts; never ship a massive PR. (We combined TQ-002+003 once — defensible as the first interactive milestone, but the lesson is to scope artifacts so one maps to one tiny PR.)
