# PR-017 — Diagonal (8-direction) movement
Status: ready · Depends on: PR-016 · Scope: ~S · Touches: src/input/input.ts (+ src/input/input.test.ts)

## Context
Beta-test feedback (2026-06-15): two directions held together should move **diagonally**. Today input maps each key to a single orthogonal step and never combines them, so the `@` only goes in 4 directions. [PR-003's note](PR-003-movement-camera.md) already flagged this ("Decide diagonal movement now — 8-dir feels better for a swarm dodger"); this ticket is that decision, made: **yes, 8-direction.** It's the natural follow-on to [PR-016](PR-016-input-responsiveness.md), whose held-direction state lets us see which directions are held *at the same time*.
...
## Plan
1. In `drain()`, after PR-016's held-direction resolution, collapse the held set into a **single** intent: `dx = (right?1:0) - (left?1:0)`, `dy = (down?1:0) - (up?1:0)`.
...
- Depends on PR-016 strictly — build it first so "which directions are held right now" is already modelled.
