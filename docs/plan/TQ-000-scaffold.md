...
Depends on: none · Scope: ~S · Touches: package.json, tsconfig.json, src/cli.ts

## Context
Foundation for everything ([tdd §1,4](../tdd.md#1-stack)). Establishes the TS + terminal-kit + rot.js toolchain.

## Goal
Merged means: `npm run dev` opens a full-screen terminal, draws a placeholder, and **restores the terminal cleanly** on `q`/Ctrl-C.
...
- [ ] `npm run dev` enters fullscreen, prints "Terminal Quest", and on quit **restores the terminal** (cursor shown, screen normal — no garbled state)
- [ ] `npm run typecheck` passes with `strict: true`
- [ ] `bin` field set for future `npx`

## Plan
1. `npm init`; add deps above.
